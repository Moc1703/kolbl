  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 font-sans">
      ...
      <div className="mb-6 border-b-2 border-gray-200 pb-4">
        <a href="/indikasi" className="text-amber-600 hover:text-amber-700 text-xs font-bold uppercase tracking-widest flex items-center gap-1 mb-2">
          <span>←</span> KEMBALI
        </a>
        ...
        <p className="text-gray-500 text-xs font-mono mt-1 uppercase tracking-widest">Pre-Warning Database Archive</p>
      </div>
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 scrollbar-none">
        {(['all', 'Lelet', 'Hilang', 'Ghost', 'Lainnya'] as const).map(f => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-sm text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-colors border ${
              filter === f
                ? f === 'all' ? 'bg-amber-600 text-white border-amber-500' : 'bg-amber-50 border-amber-200 text-amber-700'
        ...
        <select
          ...
          className="ml-auto px-3 py-2 bg-white border border-gray-200 rounded-sm text-xs font-mono text-gray-600 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 uppercase cursor-pointer shadow-sm"
        >
      ...
      {loading ? (
        <div className="text-center py-16 bg-white border border-gray-200 rounded-sm shadow-sm">
          <div className="inline-block animate-spin rounded-full h-8 w-8 border-4 border-gray-100 border-t-amber-500"></div>
          ...
          {data.map((item) => {
            ...
            return (
              <div
                ...
                className="group bg-white border border-gray-200 rounded-sm p-3 md:p-4 hover:border-amber-300 hover:bg-amber-50/10 transition-colors cursor-pointer relative overflow-hidden flex items-center gap-3 shadow-sm"
              >
                {/* Indicator Tape */}
                ...
                <div className="flex-1 min-w-0 pl-2">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-gray-900 truncate text-sm md:text-base group-hover:text-amber-700 transition-colors uppercase">{item.nama}</h3>
                  ...
                  <div className="flex items-center gap-2 text-xs font-mono text-gray-500 truncate mt-1">
                    ...
                    {!item.instagram && !item.no_hp && item.tiktok && <span className="bg-gray-100 border border-gray-200 px-1.5 py-0.5 rounded-sm text-gray-600">TT:@{item.tiktok}</span>}
                  </div>
                </div>
                
                {/* Arrow */}
                <div className="text-gray-400 group-hover:text-amber-600 transition-colors mr-2">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white border border-gray-300 rounded-sm max-w-lg w-full max-h-[90vh] shadow-2xl animate-zoom-in flex flex-col overflow-hidden">
            ...
            <div className={`p-4 md:p-6 border-b-4 flex justify-between items-start border-b-amber-500 bg-amber-50`}>
              ...
              <button 
                onClick={() => setSelected(null)} 
                className="p-2 bg-white border border-gray-200 hover:bg-amber-100 hover:text-amber-600 rounded-sm transition-colors text-gray-400 shadow-sm"
              >
            ...
            <div className="p-4 md:p-6 overflow-y-auto custom-scrollbar" style={{ paddingBottom: 'calc(1.5rem + 76px)' }}>
              ...
              <div className="mb-2">
                ...
                <div className="bg-white text-gray-700 text-sm leading-relaxed p-4 rounded-sm border-l-2 border-l-amber-500 border border-gray-200 font-serif italic shadow-sm">
                  {selected.alasan}
                </div>
              </div>
            </div>
...
Untuk pengajuan banding silahkan isi form banding dan melakukan klarifikasi terhadap pihak yang di rugikan. Penghapusan Blacklist dilakukan apabila pihak yang dirugikan telah menyatakan bahwa masalah telah selesai`;
                ...
                className="w-full py-4 bg-amber-600 text-white rounded-sm font-bold uppercase tracking-widest hover:bg-amber-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-sm"
              >
                COPY DATA / SHARE
              </button>
            </div>
      ...
      {/* CTA */}
      <div className="mt-8 bg-white border-dashed border-2 border-amber-200 p-6 text-center flex flex-col items-center rounded-sm shadow-sm">
        <p className="text-gray-500 mb-4 text-sm font-medium">
          Ada info entitas bermasalah yang belum tercatat?
        </p>
        <a 
          href="/indikasi/lapor" 
          className="inline-flex items-center justify-center px-6 py-2 bg-gray-50 text-gray-900 border border-gray-200 rounded-sm hover:bg-amber-50 hover:border-amber-300 hover:text-amber-700 transition-colors uppercase tracking-widest text-xs font-bold shadow-sm"
        >
