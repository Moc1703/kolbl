  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 font-sans">
      ...
      <div className="mb-6 border-b-2 border-neutral-800 pb-4">
        <a href="/indikasi" className="text-amber-600 hover:text-amber-500 text-xs font-bold uppercase tracking-widest flex items-center gap-1 mb-2">
          <span>←</span> KEMBALI
        </a>
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 scrollbar-none">
        {(['all', 'Lelet', 'Hilang', 'Ghost', 'Lainnya'] as const).map(f => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-sm text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-colors border ${
              filter === f
                ? f === 'all' ? 'bg-amber-700 text-white border-amber-600' : 'bg-neutral-800 border-amber-500/50 text-amber-400'
        ...
        <select
          ...
          className="ml-auto px-3 py-2 bg-neutral-900 border border-neutral-700 rounded-sm text-xs font-mono text-neutral-300 focus:outline-none focus:border-amber-500 uppercase cursor-pointer"
        >
          ...
          {data.map((item) => {
            ...
            return (
              <div
                ...
                className="group panel-dark p-3 md:p-4 hover:border-amber-900/50 hover:bg-neutral-800/50 transition-colors cursor-pointer relative overflow-hidden flex items-center gap-3"
              >
                {/* Indicator Tape */}
                ...
                {/* Arrow */}
                <div className="text-neutral-700 group-hover:text-amber-500 transition-colors mr-2">
      ...
      {selected && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-neutral-900 border border-neutral-700 rounded-sm max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-zoom-in p-0 overflow-hidden flex flex-col">
            ...
            {/* Modal Header */}
            <div className={`p-4 md:p-6 border-b-4 flex justify-between items-start border-b-amber-500 bg-amber-900/10`}>
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <p className="font-mono text-xs text-amber-500 uppercase tracking-widest font-bold">STATUS: INDIKASI MASALAH</p>
                </div>
              </div>

              <button 
                onClick={() => setSelected(null)} 
                className="p-2 bg-neutral-950 border border-neutral-800 hover:bg-amber-900/30 hover:text-amber-500 rounded-sm transition-colors text-neutral-500"
              >
...
_Dossier: Blacklist KOL Indonesia_`;
                ...
                className="w-full py-4 bg-amber-700 text-black rounded-sm font-bold uppercase tracking-widest hover:bg-amber-600 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
              >
                COPY DATA / SHARE
              </button>
            </div>
      ...
      {/* CTA */}
      <div className="mt-8 panel-dark border-dashed border-amber-900/50 p-6 text-center flex flex-col items-center">
        <p className="text-neutral-400 mb-4 text-sm font-medium">
          Ada info entitas bermasalah yang belum tercatat?
        </p>
        <a 
          href="/indikasi/lapor" 
          className="inline-flex items-center justify-center px-6 py-2 bg-neutral-800 text-white border border-neutral-700 rounded-sm hover:bg-amber-900/30 hover:border-amber-700 hover:text-amber-400 transition-colors uppercase tracking-widest text-xs font-bold"
        >
