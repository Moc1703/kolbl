  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 font-sans">
      ...
      <div className="mb-6 border-b-2 border-neutral-800 pb-4">
        <a href="/fraud" className="text-red-600 hover:text-red-500 text-xs font-bold uppercase tracking-widest flex items-center gap-1 mb-2">
          <span>←</span> KEMBALI
        </a>
        <h1 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-2">
          <span className="text-red-600">🚨</span> KASUS FRAUD
        </h1>
        <p className="text-neutral-500 text-xs font-mono mt-1 uppercase tracking-widest">Financial Crimes Database Archive</p>
      </div>
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 scrollbar-none">
        {(['all', 'Pencurian', 'Penipuan Pembayaran', 'Lainnya'] as const).map(f => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-sm text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-colors border ${
              filter === f
                ? f === 'all' ? 'bg-red-700 text-white border-red-600' : 'bg-neutral-800 border-red-500/50 text-red-400'
        ...
        <select
          ...
          className="ml-auto px-3 py-2 bg-neutral-900 border border-neutral-700 rounded-sm text-xs font-mono text-neutral-300 focus:outline-none focus:border-red-500 uppercase cursor-pointer"
        >
      ...
      {/* List */}
      {loading ? (
        <div className="text-center py-16 panel-dark">
          <div className="inline-block animate-spin rounded-full h-8 w-8 border-4 border-neutral-800 border-t-red-600"></div>
          <p className="text-neutral-500 mt-4 font-mono text-xs uppercase tracking-widest">Syncing Database...</p>
        </div>
      ) : data.length === 0 ? (
        <div className="panel-dark border-dashed border-neutral-700 p-12 text-center">
          <div className="text-4xl mb-4 opacity-50">📭</div>
          <p className="text-neutral-400 font-mono text-sm uppercase">DATABASE KOSONG</p>
        </div>
      ) : (
          ...
          {data.map((item) => {
            ...
            return (
              <div
                ...
                className="group panel-dark p-3 md:p-4 hover:border-red-900/50 hover:bg-neutral-800/50 transition-colors cursor-pointer relative overflow-hidden flex items-center gap-3"
              >
                {/* Indicator Tape */}
                ...
                <div className="flex-1 min-w-0 pl-2">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-white truncate text-sm md:text-base group-hover:text-red-400 transition-colors uppercase">{item.nama}</h3>
                    <span className={`px-1.5 py-0.5 rounded-sm text-[9px] font-bold uppercase tracking-widest shrink-0 border border-neutral-700/50 ${colors.bg} ${colors.text} bg-opacity-20 text-opacity-90`}>
                      {item.jenis_fraud === 'Penipuan Pembayaran' ? 'PENIPUAN' : item.jenis_fraud.toUpperCase()}
                    </span>
                    {item.jumlah_laporan > 1 && (
                      <span className="px-1.5 py-0.5 rounded-sm text-[9px] font-bold shrink-0 bg-red-900/30 text-red-500 border border-red-900/50">
                        {item.jumlah_laporan} OFFENSES
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 text-xs font-mono text-neutral-500 truncate mt-1">
                    {item.nominal_total > 0 && <span className="text-red-500 font-bold mr-1">{formatRupiah(item.nominal_total)}</span>}
                    {item.instagram && <span className="bg-black px-1.5 py-0.5 rounded-sm text-neutral-400">IG:@{item.instagram}</span>}
                    {item.no_hp && <span className="bg-black px-1.5 py-0.5 rounded-sm text-neutral-400">PH:{item.no_hp}</span>}
                  </div>
                </div>
                
                {/* Arrow */}
                <div className="text-neutral-700 group-hover:text-red-500 transition-colors mr-2">
      ...
      {selected && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-neutral-900 border border-neutral-700 rounded-sm max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-zoom-in p-0 overflow-hidden flex flex-col">
            ...
            {/* Modal Header */}
            <div className={`p-4 md:p-6 border-b-4 flex justify-between items-start border-b-red-600 bg-red-900/10`}>
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <p className="font-mono text-xs text-red-500 uppercase tracking-widest font-bold">STATUS: CRITICAL WARNING</p>
                </div>
              </div>
              ...
              <div className="grid gap-3 mb-6 bg-black rounded-sm p-4 border border-neutral-800">
                {selected.nominal_total > 0 && (
                  <div className="flex justify-between items-center py-2 border-b border-neutral-900 last:border-0">
                    <span className="text-neutral-500 text-xs font-bold uppercase tracking-wider">FINANCIAL LOSS</span>
                    <span className="font-bold text-red-500 text-lg">{formatRupiah(selected.nominal_total)}</span>
                  </div>
                )}
                {selected.no_hp && (
                  <div className="flex justify-between items-center py-2 border-b border-neutral-900 last:border-0">
...
_Dossier: Blacklist KOL Indonesia_`;
      ...
      {/* CTA */}
      <div className="mt-8 panel-dark border-dashed border-red-900/50 p-6 text-center flex flex-col items-center">
        <p className="text-neutral-400 mb-4 text-sm font-medium">
          Ada info entitas bermasalah yang belum tercatat?
        </p>
        <a 
          href="/fraud/lapor" 
          className="inline-flex items-center justify-center px-6 py-2 bg-neutral-800 text-white border border-neutral-700 rounded-sm hover:bg-red-900/30 hover:border-red-700 hover:text-red-400 transition-colors uppercase tracking-widest text-xs font-bold"
        >
          LAMPIKIRAN LAPORAN BARU
        </a>
      </div>
    </div>
  )
