    ...
    <div className="max-w-2xl mx-auto px-4 py-6 font-sans">
      ...
      <div className="mb-6 border-b-2 border-neutral-800 pb-4">
        <h1 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-2">
          <span className="text-red-600">🛑</span> DAFTAR BLACKLIST
        </h1>
        <p className="text-neutral-500 text-xs font-mono mt-1 uppercase tracking-widest">Global Database Archive</p>
      </div>
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 scrollbar-none">
        ...
        >
          SEMUA <span className={`px-1.5 py-0.5 rounded-sm text-[10px] font-mono ${filter === 'all' ? 'bg-black/30' : 'bg-neutral-800'}`}>{data.length}</span>
        </button>
        <button
        ...
        >
          KOL <span className={`px-1.5 py-0.5 rounded-sm text-[10px] font-mono ${filter === 'KOL' ? 'bg-purple-900' : 'bg-neutral-800'}`}>{data.filter(d => d.kategori === 'KOL').length}</span>
        </button>
        <button
        ...
        >
          MG <span className={`px-1.5 py-0.5 rounded-sm text-[10px] font-mono ${filter === 'MG' ? 'bg-cyan-900' : 'bg-neutral-800'}`}>{data.filter(d => d.kategori === 'MG').length}</span>
        </button>
            ...
            >
              ...
              <div className="flex-1 min-w-0 pl-2">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`px-1.5 py-0.5 rounded-sm text-[9px] font-bold uppercase tracking-widest shrink-0 border ${
                    item.kategori === 'KOL' ? 'bg-purple-900/20 text-purple-400 border-purple-800/50' : 'bg-cyan-900/20 text-cyan-400 border-cyan-800/50'
                  }`}>
                    {item.kategori}
                  </span>
                ...
                <div className="flex items-center gap-2 text-xs font-mono text-neutral-500 truncate mt-1">
                  ...
                  {!item.instagram && !item.no_hp && item.tiktok && <span className="bg-black px-1.5 py-0.5 rounded-sm text-neutral-400">TT:@{item.tiktok}</span>}
                </div>
              </div>
